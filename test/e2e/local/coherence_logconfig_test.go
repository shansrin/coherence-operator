...
func TestCoherenceLogConfigFileSpecSection(t *testing.T) {
	...
	err = f.Client.Create(goctx.TODO(), cluster.DeepCopy(), helper.DefaultCleanup(ctx))
	g.Expect(err).NotTo(HaveOccurred())
	...
	return &coherence.Images{
		UserArtifacts: &coherence.UserArtifactsImageSpec{
			ImageSpec: coherence.ImageSpec{
				Image: pointer.StringPtr(os.Getenv("TEST_USER_IMAGE")),
			},
			LibDir:    pointer.StringPtr("/files/lib"),
			ConfigDir: pointer.StringPtr("/files/conf"),
		},
	}
}
...
func getCoherenceClusterLoggingSpec(logLevel int32, loggingFile string,
	...
	if logLevel != 0 {
		return &coherence.LoggingSpec{
			Level: pointer.Int32Ptr(logLevel),
		}
	} else if len(loggingConfigMap) == 0 {
		return &coherence.LoggingSpec{
